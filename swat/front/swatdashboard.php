...
// Ensure teams config table exists (for installs that pre-date this feature)
...
// Get user's team name — prefer configured SWAT teams, fall back to first group
$team_name = '';
if ($DB->tableExists('glpi_plugin_swat_teams')) {
    $gr = $DB->query("SELECT g.name FROM glpi_groups_users gu
        INNER JOIN glpi_groups g ON g.id = gu.groups_id
        INNER JOIN glpi_plugin_swat_teams st ON st.groups_id = gu.groups_id
...
<div class="swat-card" style="margin-bottom:16px;">
    ...
    <div class="swat-card-body" style="padding:8px;">
        ...
        <div class="swat-permit-row" data-permit="<?= htmlspecialchars($permit) ?>" style="
            ...
            ?>
            <a href="<?= htmlspecialchars($new_form_url) ?>"
